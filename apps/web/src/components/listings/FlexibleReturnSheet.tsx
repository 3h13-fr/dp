...
export function FlexibleReturnSheet({
  open,
  onClose,
  centerLat,
  centerLng,
  ...
  const handleSelect = (suggestion: AddressSuggestion) => {
    ...
    if (distance > radiusKm) {
      setError(
        t('flexibleReturn.outOfRange', {
          distance: distance.toFixed(1),
          radius: radiusKm,
        }) ||
          `L'adresse est à ${distance.toFixed(1)} km, hors du rayon autorisé de ${radiusKm} km`
      );
      return;
    }
    setError(null);
  ...
  const content = (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold text-[var(--color-black)]">
        {tListing('flexibleReturnTitle') || 'Retour à une adresse différente'}
      </h3>
      <p className="text-sm text-[var(--color-gray-dark)]">
        {tListing('flexibleReturnRadiusInfo', { radius: radiusKm }) ||
          `Retour autorisé dans un rayon de ${radiusKm} km`}
      </p>
      <div>
        <label className="mb-2 block text-sm font-medium text-[var(--color-gray-dark)]">
          {t('options.returnAddress') || 'Adresse de retour'}
        </label>
        <AddressAutocomplete
          value={address}
          onChange={(v) => {
          ...
          placeholder={t('options.returnAddressPlaceholder') || 'Saisissez l\'adresse de retour'}
          className="w-full"
        ...
        {isValid && (
          <p className="mt-2 text-sm text-green-600">
            {t('options.returnValid') || 'Adresse valide pour le retour'}
          </p>
        )}
      </div>
      <MapWithRadius
      ...
      <>
        ...
        >
          <div className="mb-4 flex items-center justify-between">
            <h2 id="return-sheet-title" className="text-lg font-semibold">
              {tListing('flexibleReturnTitle') || 'Retour à une adresse différente'}
            </h2>
            <button
              type="button"
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div className="mb-4 flex items-center justify-between">
            <h2 id="return-modal-title" className="text-lg font-semibold">
              {tListing('flexibleReturnTitle') || 'Retour à une adresse différente'}
            </h2>
            <button
              type="button"
