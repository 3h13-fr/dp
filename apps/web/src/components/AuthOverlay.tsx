...
export function AuthOverlay({ open, onClose, children, 'aria-label': ariaLabel }: AuthOverlayProps) {
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <>
      ...
      <div
        className="fixed left-1/2 top-1/2 z-50 hidden w-full max-w-sm -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-xl md:flex"
        role="dialog"
        aria-modal="true"
        aria-label={ariaLabel ?? 'Authentication'}
      >
